...
class ReadmeToWordConverter:
    ...
    def convert(self, readme_content: str, output_filename: str, 
                include_toc: bool = True, diagram_style: str = "default") -> str:
        ...
        content_with_images = self._process_mermaid_diagrams(readme_content, diagram_style)
    ...
    def _add_table_of_contents(self, doc: Document):
        ...
        p = doc.add_paragraph()
        p.add_run("(Table of contents will be generated when you open the document in Word)")
        p.italic = True
        doc.add_page_break()
    ...
    def _process_mermaid_diagrams(self, content: str, style: str = "default") -> str:
        ...
        if self.debug_mode:
            print(f"🔄 Mermaid processing complete. Converted {self.stats['mermaid_diagrams']} diagrams")
    ...
    def _mermaid_to_image(self, mermaid_code: str, style: str = "default") -> str:
        ...
        try:
            ...
            headers = {
                'User-Agent': 'README-to-Word-Converter/1.0'
            }
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    if self.debug_mode:
                        print(f"   💾 Image saved: {image_path} ({image_path.stat().st_size} bytes)")
    ...
    def _process_single_inline_element(self, content, paragraph):
        ...
        if content.name == 'strong' or content.name == 'b':
            ...
            run = paragraph.add_run(content.get_text())
            run.font.color.rgb = None  # Blue color would need more complex implementation
        else:
        ...
        try:
            ...
            if src:
                ...
                if image_path.exists():
                    if self.debug_mode:
                        print(f"   ✅ Image found: {image_path} ({image_path.stat().st_size} bytes)")
                    ...
                    try:
                        ...
                        if self.debug_mode:
                            print(f"   ❌ Error adding image to document: {img_add_error}")
                        ...
                        try:
                            ...
                            if self.debug_mode:
                                print(f"   ❌ Failed to add image even with smaller size")
                else:
                    if self.debug_mode:
