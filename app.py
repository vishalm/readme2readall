...
def main():
    ...
    with st.sidebar:
        ...
        diagram_style = st.selectbox("🎨 Diagram Theme", ["default", "neutral", "dark", "forest"])
    ...
    with col1:
        st.markdown('<div class="content-section"><div class="section-title">📝 Input Content</div></div>', unsafe_allow_html=True)
...
Ready to convert!"""
    ...
    with col2:
        st.markdown('<div class="content-section"><div class="section-title">👀 Preview & Convert</div></div>', unsafe_allow_html=True)
        
        if uploaded_file is not None:
            readme_content = uploaded_file.read().decode('utf-8')
            with st.expander("📖 File Preview", expanded=True):
                st.code(readme_content[:500] + "..." if len(readme_content) > 500 else readme_content, language="markdown")
        
        if readme_content.strip():
            with st.expander("🔍 Markdown Preview"):
                st.markdown(readme_content[:800] + "..." if len(readme_content) > 800 else readme_content)
        ...
        if readme_content.strip():
            ...
            if st.button("🔄 Convert to Word", type="primary", use_container_width=True):
                try:
                    ...
                    output_path = converter.convert(readme_content, output_filename, include_toc, diagram_style)
